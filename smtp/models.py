...
class SMTPSettings(BaseSiteSetting):
    ...
    null=True,  # Autorise les valeurs nulles en base de données
    blank=True,  # Autorise les valeurs vides dans les formulaires
    default=587,  # Exemple de port SMTP par défaut
    ...
    def test_smtp_connection(self):
        ...
        try:
            # Test de la résolution DNS pour email_host
            import socket
            resolved = socket.gethostbyname(self.email_host)
            print(f"Adresse IP résolue pour {self.email_host}: {resolved}")
            ...
            return False, f"Impossible de résoudre le nom d'hôte {self.email_host}. Erreur: {e}"
